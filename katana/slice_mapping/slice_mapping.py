...
class SliceMapper:
    def __init__(self):
        ...
        self.nest_fields = NEST_FIELDS
        
    def map_slice(self, slice_request):
        """Maps slice request to network functions and infrastructure"""
        
        # Validate request
        if not self._validate_request(slice_request):
            return "Invalid slice request", 400
            
        # Create NEST mapping
        nest = self._create_nest_mapping(slice_request)
        
        # Determine slice type and generation
        sst, gen = self._determine_slice_type(nest)
        nest['sst'] = sst
        
        # Map network functions
        functions = self._map_network_functions(nest, gen)
        if not functions:
            return "No suitable network functions found", 400
        nest['functions'] = functions
        
        # Map infrastructure requirements  
        infra = self._map_infrastructure(nest)
        nest['infrastructure'] = infra
        
        return nest, 0
        
    def _validate_request(self, req):
        """Validates slice request has required fields"""
        try:
            base_desc = req['base_slice_descriptor']
            if not base_desc:
                return False
            for field in self.req_fields:
                if field not in base_desc:
                    return False
            return True
        except KeyError:
            return False
            
    def _create_nest_mapping(self, req):
        """Creates initial NEST mapping"""
        nest = {
            '_id': req['_id'],
            'slice_name': req['base_slice_descriptor']['base_slice_des_id'],
            'shared': {
                'isolation': req['base_slice_descriptor'].get('isolation', 0),
                'simultaneous_nsi': req['base_slice_descriptor'].get('simultaneous_nsi', 1)
            }
        }
        return nest