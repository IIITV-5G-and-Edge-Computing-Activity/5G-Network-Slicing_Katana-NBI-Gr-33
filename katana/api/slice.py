...
from katana.shared_utils.infraUtils.infraCreator import InfrastructureCreator
from katana.slice_mapping.slice_mapping import SliceMapper
...
class SliceView(FlaskView):
    ...
    def __init__(self):
        try:
            ...
            current_app.logger.error(f"Failed to import required modules: {str(e)}")
            raise
    ...
    def post(self):
        ...
        try:
            ...
            nest, error_code = self.slice_mapping.nest_mapping(request.json)
            if error_code:
                return nest, error_code

            producer = self.kafka_utils.create_producer()
